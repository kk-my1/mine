...
class prisa_model(nn.Module):
    def __init__(self,cfg):
        ...
        self.encoder_uni_t = nn.Conv1d(self.cfg.t_hid_dim,self.cfg.t_hid_dim,kernel_size=1,padding=0,bias=False)
        self.encoder_uni_a = nn.Conv1d(self.cfg.a_hid_dim,self.cfg.a_hid_dim,kernel_size=1,padding=0,bias=False)
        self.encoder_uni_v = nn.Conv1d(self.cfg.v_hid_dim,self.cfg.v_hid_dim,kernel_size=1,padding=0,bias=False)
        self.encoder_com = nn.Conv1d(self.cfg.t_hid_dim,self.cfg.t_hid_dim,kernel_size=1,padding=0,bias=False)
    ...
    def forward(self,input_dict):
        ...
        proj_x_t = x_t.permute(0, 2, 1)   # [B, D, T]
        proj_x_a = x_a.permute(0, 2, 1)
        proj_x_v = x_v.permute(0, 2, 1)

        # specific / unique
        s_t = self.encoder_uni_t(proj_x_t)   # [B, D, T]
        s_a = self.encoder_uni_a(proj_x_a)
        s_v = self.encoder_uni_v(proj_x_v)

        # common / shared
        c_t = self.encoder_com(proj_x_t)     # [B, D, T]
        c_a = self.encoder_com(proj_x_a)
        c_v = self.encoder_com(proj_x_v)

        # decoupling loss
        dec_loss = (
            self.compute_decoupling_loss(s_t, c_t) +
            self.compute_decoupling_loss(s_a, c_a) +
            self.compute_decoupling_loss(s_v, c_v)
        )

        # 转成 transformer 更容易用的 [B, T, D]
        ...
        # private branch 用 s_a / s_v
        x_v_p = s_v + self.embed_positions(s_v[:, :, 0])
        x_a_p = s_a + self.embed_positions(s_a[:, :, 0])
        ...
        x_v = x_v + self.embed_positions(x_v[:, :, 0])
        x_a = x_a + self.embed_positions(x_a[:, :, 0])
